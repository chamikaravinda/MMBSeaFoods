...
public class LFish_stockService {
	...
	public boolean UpdateStockAtRemovePurchases(ArrayList<Local_stock_items> itemsTodelete) throws SQLException {
		...
		String updateQuery = "UPDATE Local_Fish_stock set Total_Weight =?  where Fish_Type= ? ";
		try {
			for (Local_stock_items item : itemsTodelete) {
				preparedStatement = connection.prepareStatement(query);
				preparedStatement.setInt(1, item.getFish_Type());
				resultSet = preparedStatement.executeQuery();
	...
	public boolean deleteLocalStock(int id) throws SQLException {

		connection = DBConnection.Connector();
		PreparedStatement preparedStatement = null;
		int resultSet;

		String deleteQuery = "delete from Local_Fish_stock where ID=? ";
